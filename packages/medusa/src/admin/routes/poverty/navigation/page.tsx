...
const HeaderNavPage = () => {
  ...
  const hMutation = useMutation({
    mutationFn: async (newData: FormattedPovertyNavigationItems) => {
      const result = await ky
        .put("/admin/poverty/navigation/header", {
          json: newData,
        })
        .json<{ data: string | object | Array<object> }>();
      if (typeof result.data === "string") {
        throw new Error("Failed to update navigation item positions");
      }
      return result;
    },
  });
  ...
  function hUpdateItems(
    ...
    hMutation.mutate(updatedData, {
      ...
      onError: () => {
        hSetData(hData);
        toast.error("Failed to update navigation items");
      },
    });
  ...
  function fUpdateItems(
    ...
    const newData = fn(fData);
    const updatedData = newData.map((item, index) => {
      return { ...item, position: index + 1 };
    });
    fMutation.mutate(updatedData, {
      onSuccess: () => {
        toast.success("Updated navigation items");
      },
      onError: () => {
