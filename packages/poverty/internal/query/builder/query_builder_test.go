...
func TestBuildQuery(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			selects:      queryparser.Select{Fields: []string{"id", "title"}},
			expectedSQL:  "SELECT id, title FROM items WHERE status = $1",
		...
		},
		{
			...
			expectedSQL:  "SELECT id, content->>'title' as \"content.title\" FROM items WHERE content->>'category' = $1",
		...
		},
		{
			...
			expectedSQL:  "SELECT id, content->>'title' as \"content.title\", content->>'author' as \"content.author\" FROM items WHERE content->>'category' = $1 AND content->'tags' @> $2::jsonb",
		...
		},
		{
			...
			selects:      queryparser.Select{Fields: []string{"id", "content.title"}},
			expectedSQL:  "SELECT id, content->>'title' as \"content.title\" FROM items WHERE content->>'description' ILIKE $1",
		...
		},
		{
			...
			expectedSQL:  "SELECT id, content->>'title' as \"content.title\", metadata->>'status' as \"metadata.status\" FROM items WHERE metadata->>'status' = $1 AND (content->>'views')::int > $2",
		...
		},
		{
			...
			expectedSQL:  "SELECT id, content->'author'->>'email' as \"content.author.email\" FROM items WHERE content->'author'->>'name' = $1",
		...
		},
		{
			...
			expectedSQL:  "SELECT id, content->>'title' as \"content.title\", content->'author'->>'name' as \"content.author.name\", metadata->>'published_date' as \"metadata.published_date\" FROM items WHERE content->>'category' = $1 AND metadata->'tags' @> $2::jsonb AND (content->'author'->>'reputation')::float >= $3",
